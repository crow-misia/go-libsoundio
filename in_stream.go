...
func (s *InStream) Destroy() {
	if s.ptr != 0 {
		C.soundio_instream_destroy(s.pointer())
		s.ptr = 0
	}
}
...
func (s *InStream) Open() error {
	return convertToError(C.soundio_instream_open(s.pointer()))
}

// Start starts recording.
// After you call this function, ReadCallback will be called.
func (s *InStream) Start() error {
	return convertToError(C.soundio_instream_start(s.pointer()))
}
...
func (s *InStream) BeginRead(frameCount *int) (*ChannelAreas, error) {
	...
	err := convertToError(C.soundio_instream_begin_read(s.pointer(), &ptrs, &nativeFrameCount))
...
func (s *InStream) EndRead() error {
	return convertToError(C.soundio_instream_end_read(s.pointer()))
}
...
func (s *InStream) Pause(pause bool) error {
	return convertToError(C.soundio_instream_pause(s.pointer(), C.bool(pause)))
}
